...
function set_environment {
    ...
    # extra env
    if [ "${LLM_ADDITION_ENV}" != "" ];then
        echo ${LLM_ADDITION_ENV} > env.tmp
        sed -i 's/,/\n/g' env.tmp
        sed -i 's/=/="/;s/$/"/' env.tmp
...
function fetch_device_info {
    ...
    numa_nodes_num_=$(numactl -H |grep 'node [0-9]* cpus:' |wc -l)
    ...
    if [ "${numa_nodes_num}" != "${numa_nodes_num_}" ];then
    ...
    if [ "${device}" != "cuda" ];then
        device_array=($(numactl -H |grep "node [0-9]* cpus: *[0-9]+*" |sed "s/.*node//;s/cpus://" |sed -n "${numa_nodes_use_}p" |\
                awk -v cpn=${cores_per_node} '{for(i=1;i<=cpn+1;i++) {printf(" %s ",$i)} printf("\n");}' |grep '[0-9]' |\
                awk -v cpi=${cores_per_instance} -v cpn=${cores_per_node} -v cores=${LLM_TOTAL_CORES_USE} -v hi=${hbm_index} '{
...
function init_params {
    ...
    addtion_options=" ${LLM_ADDITION_PARAMS} "
    #
    for var in $@
    do
        case ${var} in
            --node_name=*)
                node_name=$(echo $var |cut -f2 -d=)
            ;;
            --device=*)
